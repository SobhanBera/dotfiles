...
drawmenu(void)
{
	...
	//temp_remove_this -> drw_setscheme(drw, scheme[SchemeNorm]);
	//temp_remove_this -> drw_text(drw, x, 0, w, bh, lrpad / 2, text, 0);
	w -= lrpad / 2;
	x += lrpad / 2;

	//remove _this_line_ny_me -> curpos = TEXTW(text) - TEXTW(&text[cursor]);
	//remove _this_line_ny_me -> if ((curpos += lrpad / 2 - 1) < w) {
	//remove _this_line_ny_me -> drw_setscheme(drw, scheme[SchemeNorm]);
	//remove _this_line_ny_me -> drw_rect(drw, x + curpos, 2 + (bh - fh) / 2, 2, fh - 4, 1, 0);
	//remove _this_line_ny_me -> }
